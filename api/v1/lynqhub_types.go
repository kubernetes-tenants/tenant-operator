...
type MySQLSource struct {
	...
	// Table is the MySQL table name containing node data
	// +kubebuilder:validation:Required
...
type ValueMappings struct {
	// UID is the column name for the node unique identifier
	// +kubebuilder:validation:Required
	UID string `json:"uid"`
...
// LynqHubSpec defines the desired state of LynqHub.
...
// LynqHubStatus defines the observed state of LynqHub.
type LynqHubStatus struct {
	// ObservedGeneration is the generation observed by the controller
	// +optional
	ObservedGeneration int64 `json:"observedGeneration,omitempty"`

	// ReferencingTemplates is the number of LynqForms referencing this hub
	// +optional
	ReferencingTemplates int32 `json:"referencingTemplates,omitempty"`

	// Desired is the total number of LynqNode CRs that should exist
	// Calculated as: ReferencingTemplates * ActiveRows
	// +optional
	Desired int32 `json:"desired,omitempty"`

	// Ready is the number of ready LynqNode resources
	// +optional
	Ready int32 `json:"ready,omitempty"`

	// Failed is the number of failed LynqNode resources
	// +optional
	Failed int32 `json:"failed,omitempty"`

	// Conditions represent the latest available observations of the hub's state
	// +optional
	// +patchMergeKey=type
	// +patchStrategy=merge
...
// +kubebuilder:printcolumn:name="Templates",type="integer",JSONPath=".status.referencingTemplates",description="Number of referencing templates"
// +kubebuilder:printcolumn:name="Desired",type="integer",JSONPath=".status.desired",description="Total node CRs (Templates * Rows)"
// +kubebuilder:printcolumn:name="Ready",type="integer",JSONPath=".status.ready",description="Number of ready nodes"
// +kubebuilder:printcolumn:name="Failed",type="integer",JSONPath=".status.failed",description="Number of failed nodes"
...
// LynqHub is the Schema for the lynqhubs API.
type LynqHub struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   LynqHubSpec   `json:"spec,omitempty"`
	Status LynqHubStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// LynqHubList contains a list of LynqHub.
type LynqHubList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []LynqHub `json:"items"`
}

func init() {
	SchemeBuilder.Register(&LynqHub{}, &LynqHubList{})
}
