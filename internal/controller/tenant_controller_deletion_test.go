...
var _ = Describe("Tenant Deletion Regression Tests", Pending, func() {
	const (
	...
	Context("When deleting a Tenant during reconciliation", func() {
		var (
			ctx           context.Context
		...
		BeforeEach(func() {
			...
			namespace = "default"
			tenantName = "test-deletion-tenant-" + time.Now().Format("150405")
		...
		It("should delete Tenant quickly when deletion is requested during reconciliation", func() {
			By("Creating a Tenant with slow resources (long timeout)")

			// Create Tenant with a resource that has long timeout
			...
			tenant = &tenantsv1.Tenant{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						"kubernetes-tenants.org/hostOrUrl": "test.example.com",
				...
				Spec: tenantsv1.TenantSpec{
					...
					Deployments: []tenantsv1.TResource{
						{
							ID:             "slow-deployment",
							NameTemplate:   tenantName + "-slow-deploy",
			...
			By("Requesting tenant deletion while reconciliation is in progress")
			deletionStart := time.Now()
			Expect(k8sClient.Delete(ctx, tenant)).To(Succeed())

			By("Verifying tenant is deleted within reasonable time (should be much less than 300s)")
			...
			}, 10*time.Second, interval).Should(BeTrue(), "Tenant should be deleted within 10 seconds")

			deletionDuration := time.Since(deletionStart)
			Expect(deletionDuration).To(BeNumerically("<", 10*time.Second),
				"Tenant deletion should complete in less than 10 seconds, but took %v", deletionDuration)
		...
		It("should remove finalizer even if cleanup fails", func() {
			By("Creating a Tenant with resources that cannot be cleaned up")
			...
			By("Marking tenant for deletion")
			Expect(k8sClient.Delete(ctx, tenant)).To(Succeed())
			...
			Eventually(func() bool {
				...
				"Tenant should be deleted even if cleanup fails")
		})

		It("should stop applying resources immediately when deletion is detected", func() {
			By("Creating a Tenant with multiple slow resources")
			...
			// Create tenant with 3 slow deployments
			tenant = &tenantsv1.Tenant{
				ObjectMeta: metav1.ObjectMeta{
					Name:      tenantName,
					Namespace: namespace,
				...
				Spec: tenantsv1.TenantSpec{
					...
					Deployments: []tenantsv1.TResource{
						{
							ID:             "deploy1",
							NameTemplate:   tenantName + "-deploy1",
							WaitForReady:   &waitForReady,
							TimeoutSeconds: timeoutSeconds,
							Spec:           deploymentSpec,
						},
						{
							ID:             "deploy2",
							NameTemplate:   tenantName + "-deploy2",
							WaitForReady:   &waitForReady,
							TimeoutSeconds: timeoutSeconds,
							Spec:           deploymentSpec,
						},
						{
							ID:             "deploy3",
							NameTemplate:   tenantName + "-deploy3",
							WaitForReady:   &waitForReady,
							TimeoutSeconds: timeoutSeconds,
							Spec:           deploymentSpec,
						},
			...
			By("Deleting tenant while processing resources")
			deletionStart := time.Now()
			Expect(k8sClient.Delete(ctx, tenant)).To(Succeed())
			...
			By("Verifying tenant is deleted")
			Eventually(func() bool {
				err := k8sClient.Get(ctx, types.NamespacedName{
					Name:      tenantName,
					Namespace: namespace,
		...
		It("should handle cleanup timeout gracefully", func() {
			...
			Eventually(func() bool {
				...
				"Tenant should be deleted within cleanup timeout (30s) + margin")
	...
	Context("When monitoring tenant deletion during WaitForReady", func() {
		It("should detect deletion within 1-2 seconds", func() {
			ctx := context.Background()
			namespace := "default"
			tenantName := "test-wait-deletion-" + time.Now().Format("150405")
			...
			// Create tenant with slow deployment
			deploymentSpec := createSlowDeploymentSpec()
			...
			tenant := &tenantsv1.Tenant{
				...
				Spec: tenantsv1.TenantSpec{
					...
					Deployments: []tenantsv1.TResource{
						{
							ID:             "slow-deployment",
							NameTemplate:   tenantName + "-deploy",
							WaitForReady:   &waitForReady,
							TimeoutSeconds: timeoutSeconds,
							Spec:           deploymentSpec,
						},
			...
			Expect(k8sClient.Create(ctx, tenant)).To(Succeed())
			defer func() { _ = k8sClient.Delete(ctx, tenant) }()
			...
			By("Deleting tenant during WaitForReady")
