...
// TestReconcile_TenantNotFound tests that Reconcile handles missing Tenant gracefully
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.resource.TargetNamespace != "" && tt.resource.TargetNamespace != tt.tenant.Namespace {
				...
				assert.Equal(t, tt.tenant.Name, labels["kubernetes-tenants.org/tenant"])
				assert.Equal(t, tt.tenant.Namespace, labels["kubernetes-tenants.org/tenant-namespace"])
			}
		})
	}
}
...
func TestCleanupTenantResources(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			err := r.cleanupTenantResources(ctx, tt.tenant)
