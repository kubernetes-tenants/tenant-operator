...
const (
	...
	FinalizerLynqHub = "lynq.sh/registry-finalizer"
)
...
func (r *LynqHubReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	existingTenants, err := r.getExistingLynqNodes(ctx, registry)
	if err != nil {
	...
	for key, tenant := range existing {
		if _, stillExists := desired[key]; !stillExists {
			...
			if err := r.Delete(ctx, tenant); err != nil {
				if !errors.IsNotFound(err) {
					logger.Error(err, "Failed to delete LynqNode", "tenant", tenant.Name, "template", key.TemplateName, "uid", key.UID)
					r.Recorder.Eventf(registry, corev1.EventTypeWarning, "TenantDeletionFailed",
						"Failed to delete LynqNode '%s': %v", tenant.Name, err)
				}
			} else {
				deletedCount++
...
func (r *LynqHubReconciler) queryDatabase(ctx context.Context, registry *lynqv1.LynqHub) ([]datasource.TenantRow, error) {
...
func (r *LynqHubReconciler) createLynqNode(ctx context.Context, registry *lynqv1.LynqHub, tmpl *lynqv1.LynqForm, row datasource.TenantRow) error {
	logger := log.FromContext(ctx)
	...
	tenant := &lynqv1.LynqNode{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"lynq.sh/registry": registry.Name,
				"lynq.sh/uid":      row.UID,
			},
	...
	logger.Info("Creating LynqNode", "tenant", tenant.Name, "uid", row.UID, "template", tmpl.Name)
...
func (r *LynqHubReconciler) shouldUpdateLynqNode(ctx context.Context, registry *lynqv1.LynqHub, tenant *lynqv1.LynqNode, row datasource.TenantRow) bool {
	// Check if stored data differs from current row data
	storedHostOrURL := tenant.Annotations["lynq.sh/hostOrUrl"]
	storedActivate := tenant.Annotations["lynq.sh/activate"]
	storedExtraJSON := tenant.Annotations["lynq.sh/extra"]
	...
	storedTemplateGeneration := tenant.Annotations["lynq.sh/template-generation"]
...
func (r *LynqHubReconciler) updateLynqNode(ctx context.Context, registry *lynqv1.LynqHub, tmpl *lynqv1.LynqForm, tenant *lynqv1.LynqNode, row datasource.TenantRow) error {
	logger := log.FromContext(ctx)

	// Check what triggered the update
	oldTemplateGeneration := tenant.Annotations["lynq.sh/template-generation"]
	newTemplateGeneration := fmt.Sprintf("%d", tmpl.Generation)
	templateChanged := oldTemplateGeneration != newTemplateGeneration
	dataChanged := tenant.Annotations["lynq.sh/hostOrUrl"] != row.HostOrURL ||
		tenant.Annotations["lynq.sh/activate"] != row.Activate
...
func (r *LynqHubReconciler) getExistingLynqNodes(ctx context.Context, registry *lynqv1.LynqHub) (*lynqv1.LynqNodeList, error) {
	tenantList := &lynqv1.LynqNodeList{}
...
func (r *LynqHubReconciler) countLynqNodeStatus(ctx context.Context, registry *lynqv1.LynqHub) (int32, int32) {
	tenants, err := r.getExistingLynqNodes(ctx, registry)
	if err != nil {
		return 0, 0
	}
...
func (r *LynqHubReconciler) cleanupRetainResources(ctx context.Context, registry *lynqv1.LynqHub) error {
	...
	for _, tenant := range tenants.Items {
		if err := r.processRetainResourcesForLynqNode(ctx, &tenant); err != nil {
...
func (r *LynqHubReconciler) processRetainResourcesForLynqNode(ctx context.Context, tenant *lynqv1.LynqNode) error {
	logger := log.FromContext(ctx)
...
func (r *LynqHubReconciler) SetupWithManager(mgr ctrl.Manager, concurrency int) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		// Watch LynqForms to re-sync Tenants when template changes
