...
func (r *LynqNodeReconciler) applyResources(ctx context.Context, tenant *lynqv1.LynqNode, sortedNodes []*graph.Node, vars template.Variables) (readyCount, failedCount, changedCount, conflictedCount int32) {
	logger := log.FromContext(ctx)
	...
	for _, node := range sortedNodes {
		...
		if err := r.Get(ctx, client.ObjectKeyFromObject(tenant), currentLynqNode); err != nil {
			if errors.IsNotFound(err) {
		...
		if changed {
			...
			if !progressingSet {
				r.StatusManager.PublishProgressingCondition(tenant, true, "Reconciling", "Reconciling changed resources")
				progressingSet = true
...
func (r *LynqNodeReconciler) emitTemplateAppliedEvent(ctx context.Context, tenant *lynqv1.LynqNode, totalResources int32) {
	logger := log.FromContext(ctx)

	// Get template information from tenant
	templateName := tenant.Spec.TemplateRef
	templateGeneration := tenant.Annotations["lynq.sh/template-generation"]

	// Count resources by type
	resourceCounts := r.countLynqNodeResourcesByType(tenant)
	resourceDetails := r.formatLynqNodeResourceDetails(resourceCounts)

	// Get registry name from labels
	registryName := tenant.Labels["lynq.sh/registry"]
	if registryName == "" {
...
func (r *LynqNodeReconciler) emitTemplateAppliedCompleteEvent(ctx context.Context, tenant *lynqv1.LynqNode, totalResources, readyCount, failedCount, changedCount int32) {
	logger := log.FromContext(ctx)
...
func (r *LynqNodeReconciler) countLynqNodeResourcesByType(tenant *lynqv1.LynqNode) map[string]int {
...
func (r *LynqNodeReconciler) buildTemplateVariablesFromAnnotations(tenant *lynqv1.LynqNode) (template.Variables, error) {
	// Get required values from annotations
	hostOrURL := tenant.Annotations["lynq.sh/hostOrUrl"]
	if hostOrURL == "" {
		hostOrURL = tenant.Spec.UID
	}

	activate := tenant.Annotations["lynq.sh/activate"]
	if activate == "" {
		activate = AnnotationValueTrue
	}

	// Parse extra values from JSON
	extraJSON := tenant.Annotations["lynq.sh/extra"]
...
func (r *LynqNodeReconciler) collectResourcesFromLynqNode(tenant *lynqv1.LynqNode) []lynqv1.TResource {
	...
	resources = append(resources, tenant.Spec.DaemonSets...)
...
func (r *LynqNodeReconciler) renderResource(ctx context.Context, engine *template.Engine, resource lynqv1.TResource, vars template.Variables, tenant *lynqv1.LynqNode) (*unstructured.Unstructured, error) {
	...
	if isCrossNamespace || isNamespaceResource {
		labels["lynq.sh/tenant"] = tenant.Name
		labels["lynq.sh/tenant-namespace"] = tenant.Namespace
	}
...
func (r *LynqNodeReconciler) cleanupLynqNodeResources(ctx context.Context, tenant *lynqv1.LynqNode) error {
	logger := log.FromContext(ctx)
	logger.Info("Starting node resource cleanup", "node", tenant.Name)
	...
	allResources := r.collectResourcesFromLynqNode(tenant)
...
func (r *LynqNodeReconciler) SetupWithManager(mgr ctrl.Manager, concurrency int) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		// These use labels for tracking: lynq.sh/tenant and lynq.sh/tenant-namespace
		Watches(
			&corev1.Namespace{},
...
// findTenantForLabeledResource maps any resource to its LynqNode using tracking labels
// This supports cross-namespace resources and resources without ownerReference support (like Namespaces)
func (r *LynqNodeReconciler) findTenantForLabeledResource(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	tenantName := labels["lynq.sh/tenant"]
	tenantNamespace := labels["lynq.sh/tenant-namespace"]
...
func (r *LynqNodeReconciler) buildAppliedResourceKeys(ctx context.Context, tenant *lynqv1.LynqNode) (map[string]bool, error) {
...
func (r *LynqNodeReconciler) deleteOrphanedResource(ctx context.Context, tenant *lynqv1.LynqNode, key string) error {
	logger := log.FromContext(ctx)
...
func (r *LynqNodeReconciler) determineReconcileType(tenant *lynqv1.LynqNode) ReconcileType {
	// 1. Check deletion
	...
	// 2. Check finalizer
	if !controllerutil.ContainsFinalizer(tenant, LynqNodeFinalizer) {
...
func (r *LynqNodeReconciler) hasOwnershipConflict(obj *unstructured.Unstructured, tenant *lynqv1.LynqNode) bool {
	...
	if len(ownerRefs) == 0 {
		...
		labels := obj.GetLabels()
		if labels != nil {
			labelLynqNode := labels["lynq.sh/tenant"]
			labelNamespace := labels["lynq.sh/tenant-namespace"]

			// If it has our tracking labels, verify they match
			if labelLynqNode != "" || labelNamespace != "" {
				return labelLynqNode != tenant.Name || labelNamespace != tenant.Namespace
			}
		}
...
func (r *LynqNodeReconciler) reconcileCleanup(ctx context.Context, tenant *lynqv1.LynqNode, startTime time.Time) (ctrl.Result, error) {
	logger := log.FromContext(ctx)

	if controllerutil.ContainsFinalizer(tenant, LynqNodeFinalizer) {
		logger.Info("LynqNode deletion requested, starting cleanup", "tenant", tenant.Name)
		...
		if err := r.cleanupLynqNodeResources(cleanupCtx, tenant); err != nil {
		...
		controllerutil.RemoveFinalizer(tenant, LynqNodeFinalizer)
...
func (r *LynqNodeReconciler) reconcileInit(ctx context.Context, tenant *lynqv1.LynqNode) (ctrl.Result, error) {
	logger := log.FromContext(ctx)

	controllerutil.AddFinalizer(tenant, LynqNodeFinalizer)
	if err := r.Update(ctx, tenant); err != nil {
		logger.Error(err, "Failed to add finalizer")
		return ctrl.Result{}, err
	}
	logger.Info("Finalizer added to LynqNode", "tenant", tenant.Name)
...
func (r *LynqNodeReconciler) reconcileSpec(ctx context.Context, tenant *lynqv1.LynqNode, startTime time.Time) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	...
	r.StatusManager.PublishResourceCounts(tenant, statusUpdate.ReadyResources, statusUpdate.FailedResources, statusUpdate.DesiredResources, statusUpdate.ConflictedResources)
	r.StatusManager.PublishAppliedResources(tenant, statusUpdate.AppliedResources)
	for _, cond := range statusUpdate.Conditions {
		switch cond.Type {
		case ConditionTypeReady:
			r.StatusManager.PublishReadyCondition(tenant, statusUpdate.IsReady, cond.Reason, cond.Message)
		case ConditionTypeProgressing:
			r.StatusManager.PublishProgressingCondition(tenant, cond.Status == metav1.ConditionTrue, cond.Reason, cond.Message)
		case ConditionTypeConflicted:
			r.StatusManager.PublishConflictedCondition(tenant, cond.Status == metav1.ConditionTrue)
		case ConditionTypeDegraded:
			r.StatusManager.PublishDegradedCondition(tenant, statusUpdate.IsDegraded, cond.Reason, cond.Message)
		}
	}
	...
	// Publish metrics
	r.StatusManager.PublishMetrics(tenant, readyCount, failedCount, totalResources, conflictedCount, statusUpdate.Conditions, statusUpdate.IsDegraded, degradedReason)
...
func (r *LynqNodeReconciler) reconcileStatus(ctx context.Context, tenant *lynqv1.LynqNode, startTime time.Time) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	...
	statusUpdate := r.calculateLynqNodeStatus(
		...
		conflictedCount,
		totalResources,
		tenant.Status.AppliedResources, // Keep existing applied resources
