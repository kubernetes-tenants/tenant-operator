...
// TestCleanupTenantResources_Timeout tests that cleanup respects timeout
func TestCleanupTenantResources_Timeout(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, tenantsv1.AddToScheme(scheme))
	require.NoError(t, corev1.AddToScheme(scheme))

	// Create tenant with ConfigMap
	...
	start := time.Now()
	err := r.cleanupTenantResources(ctx, tenant)
...
// TestApplyResources_DeletionCheck tests that applyResources checks for tenant deletion
func TestApplyResources_DeletionCheck(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, tenantsv1.AddToScheme(scheme))
	require.NoError(t, corev1.AddToScheme(scheme))

	// Create tenant that will be marked for deletion
	now := metav1.Now()
	tenant := &tenantsv1.Tenant{
		ObjectMeta: metav1.ObjectMeta{
			Name:              "test-tenant",
			Namespace:         "default",
			Finalizers:        []string{TenantFinalizer}, // Finalizer required for deletionTimestamp
			DeletionTimestamp: &now,                      // Tenant is being deleted
			Annotations: map[string]string{
				"kubernetes-tenants.org/hostOrUrl": "test.example.com",
				"kubernetes-tenants.org/activate":  "true",
				"kubernetes-tenants.org/extra":     "{}",
	...
	ready, failed, changed, conflicted := r.applyResources(ctx, tenant, nodes, vars)
...
func TestCleanupWithDeletionPolicyRetain(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(tenant, cm).Build()
	recorder := record.NewFakeRecorder(100)
...
func TestFinalizerRemovalOnCleanupFailure(t *testing.T) {
	...
	// Create tenant with malformed template that will fail cleanup
	cmSpec := &unstructured.Unstructured{
		Object: map[string]interface{}{
			"apiVersion": "v1",
			"kind":       "ConfigMap",
