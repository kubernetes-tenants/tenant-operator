...
var _ = Describe("Tenant Controller - IgnoreFields [Pending: Requires running controller]", func() {
	// NOTE: These integration tests require a running Tenant controller
	...
	Context("When reconciling a Tenant with ignoreFields for Deployment replicas", func() {
		PIt("should create resource with all fields initially, then preserve manually changed replicas while updating other fields", func() {
			...
			// Given: A Tenant with a Deployment that ignores replicas field
					...
					Deployments: []tenantsv1.TResource{
						{
							...
							CreationPolicy: tenantsv1.CreationPolicyWhenNeeded, // Default - continues syncing
			...
			// ========================================
			By("Updating Tenant template to change image to nginx:1.21")

			Eventually(func() error {
				// Get latest Tenant
				latestTenant := &tenantsv1.Tenant{}
				if err := k8sClient.Get(ctx, types.NamespacedName{Name: tenantName, Namespace: namespace}, latestTenant); err != nil {
					return err
				}

				// Update image in spec
				deploymentSpec := latestTenant.Spec.Deployments[0].Spec.Object
				...
				return k8sClient.Update(ctx, latestTenant)
			}, timeout, interval).Should(Succeed())
	...
	Context("When reconciling a Tenant with multiple ignoreFields", func() {
		PIt("should preserve multiple manually changed fields while updating others", func() {
			...
			// Given: A Tenant with multiple ignored fields
			...
			// ========================================
			By("Updating Tenant template to change image")
	...
	Context("When ignoreFields is used with CreationPolicy Once", func() {
		PIt("should behave like Once policy (ignoreFields has no effect)", func() {
			...
			// Given: A Tenant with CreationPolicy=Once and ignoreFields
					...
					Deployments: []tenantsv1.TResource{
						{
							...
							CreationPolicy: tenantsv1.CreationPolicyOnce, // Once policy
	...
	Context("When ignoreFields contains non-existent paths", func() {
		PIt("should not fail and continue reconciliation", func() {
			...
			// Given: A Tenant with ignoreFields pointing to non-existent paths
