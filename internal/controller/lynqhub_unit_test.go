...
// TestGetExistingTenants tests the getExistingLynqNodes function
...
func TestProcessRetainResourcesForLynqNode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.processRetainResourcesForLynqNode(ctx, tt.tenant)
...
// TestUpdateTenantWithConflictHandling tests that updateLynqNode handles conflicts gracefully with retry logic
func TestUpdateTenantWithConflictHandling(t *testing.T) {
	...
	err := r.updateLynqNode(ctx, registry, tmpl, tenant, row)
...
// TestCreateTenantIgnoresAlreadyExists tests that createLynqNode handles AlreadyExists errors gracefully
func TestCreateTenantIgnoresAlreadyExists(t *testing.T) {
	...
	assert.Error(t, err, "createLynqNode should return error when tenant already exists")
}
