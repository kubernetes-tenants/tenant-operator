...
// checkLynqNodeStatuses checks the status of all Tenants using this template
...
func (r *LynqFormReconciler) updateStatus(ctx context.Context, tmpl *lynqv1.LynqForm, validationErrors []string, totalLynqNodes, readyLynqNodes int32) {
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		appliedCondition := metav1.Condition{
			...
			Message:            fmt.Sprintf("%d/%d tenants ready", readyLynqNodes, totalLynqNodes),
			LastTransitionTime: metav1.Now(),
		}
		...
		if totalLynqNodes > 0 && readyLynqNodes == totalLynqNodes {
			...
			appliedCondition.Message = fmt.Sprintf("All %d tenants ready", totalLynqNodes)
...
func (r *LynqFormReconciler) findTemplateForLynqNode(ctx context.Context, tenant client.Object) []reconcile.Request {
	t := tenant.(*lynqv1.LynqNode)
