...
```yaml
args:
- --node-concurrency=20      # Increase Tenant reconciliation concurrency
...
```yaml
args:
- --node-concurrency=5       # Reduce Tenant reconciliation concurrency
...
kubectl get all -A -l lynq.sh/orphaned=true,lynq.sh/lynqnode=<lynqnode-name>
...
# Watch pod status
watch kubectl get pods -l lynqnode=<name>
```
...
2. Check Tenant events: `kubectl describe lynqnode <name>`
