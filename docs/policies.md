...
## ⚠️ Important: Protecting Tenants from Cascade Deletion

When you delete a TenantRegistry or TenantTemplate, all associated Tenant CRs are automatically deleted due to Kubernetes ownerReference behavior. This can result in **unintended deletion of all tenant resources**.
...
spec:
  ...
  namespaces:
    - id: tenant-ns
      deletionPolicy: Retain  # ✅ Keeps namespace even if Tenant CR is deleted
      nameTemplate: "{{ .uid }}"
...
namespaces:
  - id: ns
    creationPolicy: WhenNeeded  # Maintain namespace
    deletionPolicy: Retain      # Keep namespace for investigation
    conflictPolicy: Force       # Operator manages namespaces
...
**Rationale:**
- `WhenNeeded`: Keep namespace labels/annotations updated
- `Retain`: Namespace might have resources from other sources
- `Force`: Operator is authoritative for namespaces
