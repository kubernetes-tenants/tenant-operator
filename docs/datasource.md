...
- **username**: Database username (read-only recommended)
- **passwordRef**: Reference to Kubernetes Secret containing password
- **database**: Database name
- **table**: Table or view name containing tenant data
- **syncInterval**: How often to sync with database (e.g., `30s`, `1m`, `5m`)
...
**Invalid/Falsy Values** (tenant will be ignored):
```
❌ "0"
❌ "false"
❌ "FALSE"
...
**Important:**
- Only the exact strings above are considered active
- Boolean database columns work if they return `"1"` or `"true"` as strings
- Integer columns (TINYINT) work if they return `"1"` as string
